...
def test_scrape_failure_prefers_override_over_cache(tmp_path, _no_avatar_downloads):
    paths = _paths(tmp_path)
    paths.cache_dir.mkdir(parents=True, exist_ok=True)
    cached = PlatformStats(platform="xbox", headline_value=500)
    fetch_mod.cache_mod.save(paths.cache_dir, cached)
    paths.overrides_file.write_text(
        yaml.safe_dump({"xbox": {"headline_value": 42, "username": "Manual"}}),
        encoding="utf-8",
    )
    ...
    with patch.dict(fetch_mod.PROVIDERS, providers, clear=True):
        out = fetch_all(_cfg(xbox="http://x"), paths, scrape=True)
    assert out["xbox"].headline_value == 42
    assert out["xbox"].username == "Manual"


def test_scrape_failure_no_cache_no_override_uses_placeholder(tmp_path, _no_avatar_downloads):
    providers = {"xbox": _make_provider(error=ProviderError("fail"))}
    paths = _paths(tmp_path)
    with patch.dict(fetch_mod.PROVIDERS, providers, clear=True):
        out = fetch_all(_cfg(xbox="http://x"), paths, scrape=True)
...
def test_fresh_scrape_wins_over_override_and_cache(tmp_path, _no_avatar_downloads):
    paths = _paths(tmp_path)
    paths.cache_dir.mkdir(parents=True, exist_ok=True)
    fetch_mod.cache_mod.save(paths.cache_dir, PlatformStats(platform="xbox", headline_value=500))
    paths.overrides_file.write_text(
        yaml.safe_dump({"xbox": {"headline_value": 42}}), encoding="utf-8"
    )
...
def test_empty_profile_url_skips_provider(tmp_path, _no_avatar_downloads):
    ...
    with patch.dict(fetch_mod.PROVIDERS, providers, clear=True):
        out = fetch_all(cfg, paths, scrape=True)
    # No cache, no override, no URL — placeholder
    assert out["xbox"].username == "Gamer"
...
def test_overrides_file_seeded_on_first_run(tmp_path, _no_avatar_downloads):
    paths = _paths(tmp_path)
    providers = {"xbox": _make_provider(PlatformStats(platform="xbox", headline_value=1))}
    with patch.dict(fetch_mod.PROVIDERS, providers, clear=True):
        fetch_all(_cfg(xbox="http://x"), paths, scrape=True)
    assert paths.overrides_file.exists()
    assert "xbox" in yaml.safe_load(paths.overrides_file.read_text())


def test_existing_overrides_not_overwritten(tmp_path, _no_avatar_downloads):
    paths = _paths(tmp_path)
    paths.overrides_file.parent.mkdir(parents=True, exist_ok=True)
    original = {"xbox": {"headline_value": "DO NOT OVERWRITE"}}
    paths.overrides_file.write_text(yaml.safe_dump(original), encoding="utf-8")

    providers = {"xbox": _make_provider(PlatformStats(platform="xbox", headline_value=1))}
    with patch.dict(fetch_mod.PROVIDERS, providers, clear=True):
        fetch_all(_cfg(xbox="http://x"), paths, scrape=True)

    assert yaml.safe_load(paths.overrides_file.read_text()) == original
...
def test_cache_not_written_when_fresh_is_none_but_override_applied(tmp_path, _no_avatar_downloads):
    # Guards against `fresh is not None and merged is not None` → `or`: the
    # cache must be written ONLY when we have fresh data. An override-derived
    # result must not persist to cache.
    paths = _paths(tmp_path)
    paths.overrides_file.parent.mkdir(parents=True, exist_ok=True)
    paths.overrides_file.write_text(
        yaml.safe_dump({"xbox": {"headline_value": 77}}), encoding="utf-8"
    )
    providers = {"xbox": _make_provider(error=ProviderError("fail"))}
    with patch.dict(fetch_mod.PROVIDERS, providers, clear=True), \
